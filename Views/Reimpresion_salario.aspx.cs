...
{
    public partial class Reimpresion_salario : System.Web.UI.Page
    {
        protected int saldo = 1;
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                e.Row.Cells[11].CssClass = "boundAgreg";
        ...
        public async void imprimir_cartas_Click(object sender, EventArgs e)
        {
            ...
            List<RequestFile> filesList = new List<RequestFile>();
            foreach (GridViewRow row in GridView1.Rows)
            {
                if (row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (chkRow.Checked)
                    {
                        ...
                        if (txtbox.Text != "" || !(string.IsNullOrEmpty(txtbox.Text.Trim())))
                        {
                            var tipo_carta = row.Cells[6].Text;
                            var FecImp = row.Cells[7].Text;
                            var salario = txtbox.Text;
                            int requestID = int.Parse(row.Cells[11].Text.ToString());



                            var filePath = SendPrinter.GetFileByRequestID(requestID);
                            Session.Add("route", filePath);
                            string TemplateFileNameDir = Path.GetDirectoryName(filePath);

                            string TemplateFileNameR = Path.GetFileNameWithoutExtension(filePath);
                            filePath = TemplateFileNameDir +"\\"+ TemplateFileNameR + "_Replaced.docx";

                            string fileName = Path.GetFileName(filePath);

                            //method called to replace salary on existing file
                            isPrintSuccessful = CallPrintProcess(tipo_carta, codigo, FecImp, salario, requestID);
           ...
           log_of_files_printed = await SendPrinter.PrintDocumentAsync(filesList.ToArray());
        ...
        /*public static void conversionPDF(string MyVarNum, string tipoCarta)
        {
            ...
            try
            {
                objPres.ExportAsFixedFormat(pdfPath, Microsoft.Office.Interop.Word.WdExportFormat.wdExportFormatPDF, false, Microsoft.Office.Interop.Word.WdExportOptimizeFor.wdExportOptimizeForPrint, Microsoft.Office.Interop.Word.WdExportRange.wdExportAllDocument);
            }
            catch (Exception e)
            {
                throw e;
        ...
        private bool CallPrintProcess(string TipoCarta, string codigo, string FecImp, string salario, int rowId)
        {
            ...
            if (TipoCarta == "1")
            {
                ...
                if (existsConst)
                {
                    temp_return_result = true;
                    //SendPrinter.imprimir(1);
            ...
            if (TipoCarta == "2")
            {
                ...
                fecha_cartas.GetFechaCartas();
                existsConst = constancia.migracion(rowId);
                if (existsConst)
                {
            ...
            if (TipoCarta == "6")
            {
                ...
                //Guardar el archivo en carpeta de impresion
                template.SaveAs(printFolderPath + @"\carta_" + "_cambioClinica_#" + randNum.Next(10000, 99999) + ".docx");
        ...
        private static void ReplaceValues(string filepath, string salary, string date)
        {
        }
    }
}
